...
def listenEvents():
    ...
    if(keysDown[pygame.K_RIGHT]):
        ...
        #Put here what should be run when the compile key is pressed (right arrow)
        mainBoard = elaborateBoard(mainBoard)
...
#Function that elaborates possible entries for each slot
def evaluateEntries(board = []):

    #Array returned
    entriesBoard = createBoard()
    
    for y in range(9):
        for x in range(9):

            #Setting already determined numbers 
            if board[y][x] != 0:
                entriesBoard[y][x] = [board[y][x]]
                continue
            ...
            #Setting the remaining possible entries
            entriesBoard[y][x] = possibleEntries

    return entriesBoard

#Function that elaborates and fills out the blank spots in the board
def elaborateBoard(board = []):

    #Loop through all slots
    for y in range(9):
        for x in range(9):

            #If the current slot is not empty skip it
            if board[y][x] != 0:
                continue

            #Get possible entries
            possibleEntries = evaluateEntries(board)

            #If there's only an entry for this slot put it as the right one
            if len(possibleEntries[y][x]) == 1 :
                board[y][x] = possibleEntries[y][x][0]
                continue

    #Check if a possible entry appeares only once in each column and set it to the corresponding slot
    for x in range(9):
        ...
        for n in range(1,10):
            ...
            for y in range(9):
                ...
                #Add one if n in in possibleEntries
                if n in possibleEntries[y][x]:
                    i += 1

            #If only 1 slot has n in its possible entries
            if i == 1:

                #Search the y coordinate of that slot and set it as the right one
                for y in range(9):
                    if n in possibleEntries[y][x]:
                        board[y][x] = n

    #Check if a possible entry appeares only once in each row and set it to the corresponding slot
    for y in range(9):
        ...
        for n in range(1,10):
            ...
            if i == 1:
                ...
                #Search the x coordinate of that slot and set it as the right one
                for x in range(9):
                    if n in possibleEntries[y][x]:
                        board[y][x] = n
